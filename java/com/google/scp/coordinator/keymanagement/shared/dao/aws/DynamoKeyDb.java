...
public final class DynamoKeyDb implements KeyDb {
  ...
  private static String AND(String... expressions) {
    return String.format("(%s)", String.join(" AND ", PAREN(expressions)));
  }

  private static String OR(String... expressions) {
    return String.format("(%s)", String.join(" OR ", PAREN(expressions)));
  }
  ...
  private static String[] PAREN(String... expressions) {
    String[] parenAdded = new String[expressions.length];
    for (int i = 0; i < expressions.length; i++) {
      if (!expressions[i].startsWith("(") || !expressions[i].endsWith(")")) {
        parenAdded[i] = "(" + expressions[i] + ")";
      } else {
        parenAdded[i] = expressions[i];
      }
    }
    ...
    return StaticImmutableTableSchema.builder(EncryptionKey.class, EncryptionKey.Builder.class)
        ...
        .addAttribute(
            ...
            attribute ->
                attribute
                    .name(SET_NAME)
                    .getter(EncryptionKey::getSetName)
                    .setter(EncryptionKey.Builder::setSetName))
        .build();
  }
                  ...
                  ScanEnhancedRequest.builder()
                      .filterExpression(
                          Expression.builder()
                              .expression(
                                  AND(
                                      OR(
                                          SET_NAME + EQUAL + NAME,
                                          AND(
                                              DEFAULT_NAME + EQUAL + NAME,
                                              "attribute_not_exists(" + SET_NAME + ")")),
                              ...
                              .putExpressionValue(NAME, stringValue(setName))
                              .putExpressionValue(DEFAULT_NAME, stringValue(DEFAULT_SET_NAME))
  ...
  @Override
  public Stream<EncryptionKey> listRecentKeys(String setName, Duration maxAage)
      throws ServiceException {
    try {
                              ...
                              AND(
                                  OR(
                                      ...
                                      AND(
                                          DEFAULT_NAME + EQUAL + NAME,
                                          "attribute_not_exists(" + SET_NAME + ")")),
                                  CREATION_TIME + GREATER_OR_EQUAL + MAX))
                          .putExpressionValue(NAME, stringValue(setName))
                          .putExpressionValue(DEFAULT_NAME, stringValue(DEFAULT_SET_NAME))
