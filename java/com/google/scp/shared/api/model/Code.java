...
public enum Code {
  ...
  public static Code fromHttpStatusCode(int internalCode) {
    for (Code code : Code.values()) {
      if (code.getHttpStatusCode() == internalCode) {
        return code;
      }
    }
    throw new IllegalArgumentException("Unknown HTTP status code: " + internalCode);
  }
