...
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.JOB_KEY_COLUMN;
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.JOB_STATUS_COLUMN;
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.NUM_ATTEMPTS_COLUMN;
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.REQUEST_INFO_COLUMN;
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.REQUEST_PROCESSING_STARTED_AT;
import static com.google.scp.operator.shared.dao.metadatadb.gcp.SpannerMetadataDb.SpannerJobMetadataTableColumn.REQUEST_RECEIVED_AT_COLUMN;
...
public final class SpannerMetadataDb implements JobMetadataDb {
                ...
                Timestamp lastUpdateTime =
                    transaction
                        .readRow(
                            ...
                            List.of(REQUEST_UPDATED_AT_COLUMN.label))
                        .getTimestamp(REQUEST_UPDATED_AT_COLUMN.label)
                        .toProto();
  ...
  private JobMetadata convertResultSetToJobMetadata(ResultSet resultSet)
    ...
    JobKey jobKey =
        JobKey.newBuilder().setJobRequestId(resultSet.getString(JOB_KEY_COLUMN.label)).build();
    JobStatus jobStatus = JobStatus.valueOf(resultSet.getString(JOB_STATUS_COLUMN.label));
    String serverJobId = resultSet.getString(SERVER_JOB_ID_COLUMN.label);
    Timestamp requestReceivedAt =
        resultSet.getTimestamp(REQUEST_RECEIVED_AT_COLUMN.label).toProto();
    Timestamp requestUpdatedAt = resultSet.getTimestamp(REQUEST_UPDATED_AT_COLUMN.label).toProto();
    int numAttempts = (int) resultSet.getLong(NUM_ATTEMPTS_COLUMN.label);
    ...
    if (!resultSet.isNull(REQUEST_INFO_COLUMN.label)) { // REQUEST_INFO_COLUMN is nullable
      String serializedRequestInfo = resultSet.getJson(REQUEST_INFO_COLUMN.label);
    ...
    if (!resultSet.isNull(RESULT_INFO_COLUMN.label)) { // RESULT_INFO_COLUMN is nullable
      String serializedResultInfo = resultSet.getJson(RESULT_INFO_COLUMN.label);
    ...
    if (!resultSet.isNull(REQUEST_PROCESSING_STARTED_AT.label)) {
      Timestamp requestProcessingStartedAt =
          resultSet.getTimestamp(REQUEST_PROCESSING_STARTED_AT.label).toProto();
