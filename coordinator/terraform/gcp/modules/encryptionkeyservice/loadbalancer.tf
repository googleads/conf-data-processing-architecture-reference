...
resource "google_compute_ssl_policy" "encryption_key_service_loadbalancer_ssl_policy" {
  name            = "${var.environment}-encryption-key-service-policy"
  profile         = "MODERN"
...
resource "google_compute_target_https_proxy" "encryption_key_service_loadbalancer_proxy" {
  ...
  ssl_policy = google_compute_ssl_policy.encryption_key_service_loadbalancer_ssl_policy.self_link
  ssl_certificates = [
