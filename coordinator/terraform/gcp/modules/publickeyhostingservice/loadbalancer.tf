...
# SSL policy disabling insecure SSL/TLS versions and features.
resource "google_compute_ssl_policy" "get_public_key_loadbalancer_ssl_policy" {
  name            = "${var.environment}-get-public-key-policy"
  profile         = "MODERN"
...
resource "google_compute_target_https_proxy" "get_public_key_loadbalancer_proxy" {
  ...
  ssl_policy = google_compute_ssl_policy.get_public_key_loadbalancer_ssl_policy.self_link
  ssl_certificates = [
