...
TEST(LibCpioTest, ConsoleLogTest) {
  ...
  EXPECT_THAT(GlobalCpio::GetGlobalCpio(), NotNull());
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kEmergency));
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kAlert));
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kCritical));
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kError));
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kWarning));
  EXPECT_TRUE(GlobalLogger::IsLogLevelEnabled(LogLevel::kDebug));
  EXPECT_FALSE(GlobalLogger::IsLogLevelEnabled(LogLevel::kInfo));
  EXPECT_SUCCESS(TestLibCpio::ShutdownCpio(options));
}
