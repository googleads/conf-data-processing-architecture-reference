...
struct JobClientOptions {
  ...
  // Retry interval for reading job from database.
  std::chrono::milliseconds read_job_retry_interval =
      std::chrono::milliseconds(501);

  // The maximum retry count for reading job from database.
  size_t read_job_max_retry_count = 8;
};
}  // namespace google::scp::cpio
