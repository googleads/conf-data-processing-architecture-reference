...
struct CpioOptions {
  ...
  /// Enabled log levels. Only when the level is in this set, CPIO will print
  /// out the log of that level.
  std::unordered_set<core::LogLevel> enabled_log_levels = {
      core::LogLevel::kEmergency, core::LogLevel::kAlert,
      core::LogLevel::kCritical, core::LogLevel::kError,
      core::LogLevel::kWarning};
...
// Options for DatabaseClient.
struct DatabaseClientOptions {
  virtual ~DatabaseClientOptions() = default;

  DatabaseClientOptions() = default;

  DatabaseClientOptions(std::string input_gcp_spanner_instance_name,
                        std::string input_gcp_spanner_database_name)
      : gcp_spanner_instance_name(std::move(input_gcp_spanner_instance_name)),
        gcp_spanner_database_name(std::move(input_gcp_spanner_database_name)) {}
