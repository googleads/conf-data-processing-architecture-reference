...
class ParameterClientTest : public ScpTestBase {
 protected:
  ParameterClientTest() {
...
TEST_F(ParameterClientTest, GetParameterSuccess) {
  ...
  auto context = AsyncContext<GetParameterRequest, GetParameterResponse>(
      make_shared<GetParameterRequest>(), [&](auto& context) {
        ...
        EXPECT_THAT(*context.response, EqualsProto(GetParameterResponse()));
        finished = true;
      });
...
TEST_F(ParameterClientTest, GetParameterSyncSuccess) {
  EXPECT_CALL(*client_->GetParameterClientProvider(), GetParameter)
