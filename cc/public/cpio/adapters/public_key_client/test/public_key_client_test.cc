...
class PublicKeyClientTest : public ScpTestBase {
 protected:
  PublicKeyClientTest() {
...
TEST_F(PublicKeyClientTest, ClientProviderCalled) {
  ...
  client_->ListPublicKeys(context);
}

TEST_F(PublicKeyClientTest, ListPublicKeysSyncSuccess) {
  EXPECT_CALL(*client_->GetPublicKeyClientProvider(), ListPublicKeys)
  ...
  EXPECT_SUCCESS(client_->ListPublicKeysSync(ListPublicKeysRequest()).result());
}
