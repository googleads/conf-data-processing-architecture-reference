...
class MockAggregateMetric : public AggregateMetricInterface {
 public:
  MockAggregateMetric() {
    ...
    ON_CALL(*this, Stop)
        .WillByDefault(testing::Return(core::SuccessExecutionResult()));
    ON_CALL(*this, Increment)
        .WillByDefault(testing::Return(core::SuccessExecutionResult()));
    ON_CALL(*this, IncrementBy)
        .WillByDefault(testing::Return(core::SuccessExecutionResult()));
  }
  ...
  MOCK_METHOD(core::ExecutionResult, Increment, ((const std::string&)),
              (noexcept, override));
  MOCK_METHOD(core::ExecutionResult, IncrementBy,
              (uint64_t, const std::string&), (noexcept, override));
};
}  // namespace google::scp::cpio
