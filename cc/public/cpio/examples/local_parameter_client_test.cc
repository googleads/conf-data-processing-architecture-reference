...
int main(int argc, char* argv[]) {
  ...
  auto get_parameter_request = make_shared<GetParameterRequest>();
          ...
          [&](AsyncContext<GetParameterRequest, GetParameterResponse> context) {
            if (!context.result.Successful()) {
              ...
              std::cout << "GetParameter succeeded, and parameter is: "
                        << context.response->parameter_value() << std::endl;
            }
            finished = true;
          });
  parameter_client->GetParameter(get_parameter_context);
