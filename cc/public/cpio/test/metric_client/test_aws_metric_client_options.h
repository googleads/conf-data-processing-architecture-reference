...
struct TestAwsMetricClientOptions : public MetricClientOptions {
  virtual ~TestAwsMetricClientOptions() = default;

  TestAwsMetricClientOptions() = default;

  explicit TestAwsMetricClientOptions(const MetricClientOptions& options)
      : MetricClientOptions(options) {}

  std::string cloud_watch_endpoint_override;
};
}  // namespace google::scp::cpio
