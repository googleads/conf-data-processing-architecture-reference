  ...
  try {
    ...
    if (ec.failed()) {
      ...
      SCP_ERROR(kHttp2Client, kZeroUuid, result,
                "Failed to initialize with tls ctx error %d and message %s.",
...
void HttpConnection::SendHttpRequest(
  ...
  if (ec.failed()) {
    ...
    SCP_ERROR_CONTEXT(kHttp2Client, http_context, http_context.result,
                      "Http request for %s failed for the client with error "
                      "code %d and error message %s!",
                      uri.value().c_str(), ec.value(), ec.message().c_str());
  ...
  SCP_DEBUG_CONTEXT(
      kHttp2Client, http_context, "Sending http request for %s.",
      GetEscapedUriWithQuery(*http_context.request).value().c_str());
