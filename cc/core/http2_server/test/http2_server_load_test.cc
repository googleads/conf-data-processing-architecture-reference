 ...
 protected:
  HttpServerLoadTest() {
    ...
    shared_ptr<MetricInstanceFactoryInterface> metric_instance_factory =
        make_shared<cpio::MockMetricInstanceFactory>();

    auto options = Http2ServerOptions(
        false, make_shared<string>(), make_shared<string>(),
        core::common::RetryStrategyOptions(
            core::common::RetryStrategyType::Exponential, 31, 3),
    ...
    http_server_ = make_shared<Http2Server>(
        ...
        metric_instance_factory, config_provider_, options);
