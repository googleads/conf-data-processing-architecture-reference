...
class Http2ServerTest : public ScpTestBase {
 public:
  ...
  Http2ServerTest() {
    async_executor =
        make_shared<AsyncExecutor>(4 /* threads*/, 1000 /* queue size*/);
    mock_config_provider = make_shared<MockConfigProvider>();
    mock_metric_instance_factory = make_shared<MockMetricInstanceFactory>();

    EXPECT_SUCCESS(async_executor->Init());
    EXPECT_SUCCESS(async_executor->Run());
  }

  ~Http2ServerTest() { EXPECT_SUCCESS(async_executor->Stop()); }
...
TEST_F(Http2ServerTest, Run) {
  ...
  auto options =
      Http2ServerOptions(false, make_shared<string>(), make_shared<string>(),
                         common::RetryStrategyOptions(
                             common::RetryStrategyType::Exponential, 31, 3),
                         "TestMetricNamespace", "TestHttpServer");
...
TEST_F(Http2ServerTest, ShouldHandleRequestProperlyWhenTlsIsEnabled) {
  ...
  // Start the server
  Http2Server http_server(host_address, port, thread_pool_size, async_executor,
                          authorization_proxy, mock_metric_instance_factory,
                          mock_config_provider, http2_server_options);
