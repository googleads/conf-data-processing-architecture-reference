...
TEST(Base64Test, Base64EncodeValidValue) {
  ...
  EXPECT_THAT(Base64Encode(decoded),
              IsSuccessfulAndHolds("dGVzdF90ZXN0X3Rlc3Q="));
}

TEST(Base64Test, Base64EncodeValidValueOLD) {
  string decoded("test_test_test");
  string encoded;
...
TEST(Base64Test, Base64DecodeInvalidValue) {
  ...
  EXPECT_THAT(Base64Decode(encoded),
...
TEST(Base64Test, Base64DecodeValidValues) {
  string empty;
  EXPECT_SUCCESS(Base64Decode(empty));

  string encoded("dGVzdF90ZXN0X3Rlc3Q=");
  EXPECT_THAT(Base64Decode(encoded), IsSuccessfulAndHolds("test_test_test"));
}

TEST(Base64Test, Base64DecodeValidValuesOLD) {
  string empty;
  EXPECT_SUCCESS(Base64Decode(empty));
