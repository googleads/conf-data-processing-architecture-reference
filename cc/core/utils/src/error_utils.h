 ...
 * @brief Convert internal execution result to public result
 *
 * @param response_or the given internal execution result wrapping response.
 * @return ExecutionResult returned public execution result wrapping response.
 */
template <typename TResponse>
ExecutionResultOr<TResponse> ConvertToPublicExecutionResult(
    const ExecutionResultOr<TResponse>& response_or) {
  if (response_or.result().Successful()) {
    return response_or;
  }
  return ConvertToPublicExecutionResult(response_or.result());
}
}  // namespace google::scp::core::utils
