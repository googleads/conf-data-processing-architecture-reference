...
ExecutionResultOr<string> Base64Decode(const string& encoded) {
  ...
  return string(reinterpret_cast<char*>(buffer.get()), output_len);
}

ExecutionResultOr<string> Base64Encode(const string& decoded) {
  size_t required_len = 0;
  ...
  return string(reinterpret_cast<char*>(buffer.get()), ret);
}

ExecutionResult Base64Decode(const string& encoded, string& decoded) {
  ASSIGN_OR_RETURN(decoded, Base64Decode(encoded));
  return SuccessExecutionResult();
}

ExecutionResult Base64Encode(const string& decoded, string& encoded) {
  ASSIGN_OR_RETURN(encoded, Base64Encode(decoded));
  return SuccessExecutionResult();
}
