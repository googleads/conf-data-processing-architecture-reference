...
DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_TIMEOUT, SC_CURL_CLIENT,
                  0x000B, "HTTP server returned REQUEST_TIMEOUT",
                  HttpStatusCode::REQUEST_TIMEOUT);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_CONFLICT, SC_CURL_CLIENT, 0x000C,
                  "HTTP server returned CONFLICT", HttpStatusCode::CONFLICT);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_PRECONDITION_FAILED, SC_CURL_CLIENT,
                  0x000D, "HTTP server returned PRECONDITION_FAILED",
                  HttpStatusCode::PRECONDITION_FAILED);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_TOO_MANY_REQUESTS, SC_CURL_CLIENT,
                  0x000E, "HTTP server returned TOO_MANY_REQUESTS",
                  HttpStatusCode::TOO_MANY_REQUESTS);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_SERVER_ERROR, SC_CURL_CLIENT, 0x000F,
                  "HTTP server returned INTERNAL_SERVER_ERROR",
                  HttpStatusCode::INTERNAL_SERVER_ERROR);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_NOT_IMPLEMENTED, SC_CURL_CLIENT,
                  0x0010, "HTTP server returned NOT_IMPLEMENTED",
                  HttpStatusCode::NOT_IMPLEMENTED);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_BAD_GATEWAY, SC_CURL_CLIENT,
                  0x0021, "HTTP server returned BAD_GATEWAY",
                  HttpStatusCode::BAD_GATEWAY);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_SERVICE_UNAVAILABLE, SC_CURL_CLIENT,
                  0x0022, "HTTP server returned SERVICE_UNAVAIALABLE",
                  HttpStatusCode::SERVICE_UNAVAILABLE);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_OTHER_HTTP_ERROR, SC_CURL_CLIENT,
                  0x0023, "HTTP server returned unenumerated error",
                  HttpStatusCode::BAD_REQUEST);

DEFINE_ERROR_CODE(SC_CURL_CLIENT_REQUEST_BAD_REGEX_PARSING, SC_CURL_CLIENT,
                  0x0024, "Response code could not be parsed",
                  HttpStatusCode::BAD_REQUEST);
