...
class SingleThreadAsyncExecutor : ServiceInterface {
 public:
  explicit SingleThreadAsyncExecutor(
      ...
      : is_running_(false),
        ...
        enable_stats_keeping_(enable_stats_keeping) {}
  ...
  /**
   * @brief Returns the current size of the normal queue and the high queue.
   */
  std::pair<size_t, size_t> GetQueueSizes() const;

  /**
   * @brief Get the SingleThreadExecutorStats.
   */
  const SingleThreadExecutorStats& GetSingleThreadExecutorStats() const;
 ...
 private:
  ...
  /**
   * @brief The object to record statistics in.
   *
   */
  SingleThreadExecutorStats stats_;

  /**
   * @brief Whether to record stats or not.
   *
   */
  bool enable_stats_keeping_;
};
}  // namespace google::scp::core
