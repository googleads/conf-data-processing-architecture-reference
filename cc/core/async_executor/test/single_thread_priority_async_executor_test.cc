...
TEST(SingleThreadPriorityAsyncExecutorTests, CountWorkSingleThread) {
  ...
  // We expect that these tasks get done executing some acceptable time around
  // our scheduled time.
  auto done_executing_time =
      TimeProvider::GetSteadyTimestampInNanoseconds().count();
  const auto acceptable_buffer_time = milliseconds(20);
  const auto lower_limit = (execute_time - acceptable_buffer_time).count();
  const auto upper_limit = (execute_time + acceptable_buffer_time).count();
  EXPECT_THAT(done_executing_time, IsBetween(lower_limit, upper_limit));
  EXPECT_EQ(count, queue_cap);

  EXPECT_SUCCESS(executor.Stop());
}

class AffinityTest : public ScpTestBase,
  ...
  for (int i = 0; i < queue_cap; i++) {
    EXPECT_SUCCESS(executor.ScheduleFor(
        [&]() {
          ...
          if (GetCpu() < thread::hardware_concurrency()) {
...
INSTANTIATE_TEST_SUITE_P(SingleThreadPriorityAsyncExecutorTests, AffinityTest,
                         Values(0, 1, thread::hardware_concurrency() - 1,
