...
struct Uuid {
  ...
  // Enables usage as a key in absl maps and sets.
  template <typename H>
  friend H AbslHashValue(H h, const Uuid& o);
...
template <typename H>
H AbslHashValue(H h, const Uuid& o) {
  return H::combine(std::move(h), UuidCompare::hash(o));
}
