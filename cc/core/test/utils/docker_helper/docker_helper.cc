...
int StartGcpContainer(const string& network, const string& container_name,
                      const vector<string>& exposed_ports) {
  vector<string> port_mappings;
  for (auto& port : exposed_ports) {
    if (!port.empty()) {
      port_mappings.push_back(PortMapToSelf(port));
    }
  }
  map<string, string> env_variables;
  return StartContainer(network, container_name, kGcpImage, port_mappings,
                        env_variables);
}

int StartContainer(
    const string& network, const string& container_name,
    const string& image_name, const vector<string>& port_mappings,
    const std::map<std::string, std::string>& environment_variables,
    const string& addition_args) {
  return std::system(
      BuildStartContainerCmd(network, container_name, image_name, port_mappings,
                             environment_variables, addition_args)
          .c_str());
}
...
string BuildStartContainerCmd(
    ...
    const std::map<std::string, std::string>& environment_variables,
    const string& addition_args) {
  string port_mappings_flag;
  for (auto& port_mapping : port_mappings) {
    if (!port_mapping.empty()) {
      port_mappings_flag += absl::StrFormat("-p %s ", port_mapping);
    }
  }
  ...
  return absl::StrFormat(
      ...
      "%s"
      "%s",
      name_network, container_name, port_mappings_flag, envs,
