...
struct AsyncContext {
  ...
  AsyncContext(const AsyncContext& right) {
    ...
    convert_to_public_error = right.convert_to_public_error;
  }
  ...
  virtual void Finish() noexcept {
    if (callback) {
      ...
      if (convert_to_public_error) {
        result = utils::ConvertToPublicExecutionResult(result);
      }
      callback(*this);
    }
  }

  void setConvertToPublicError(bool convert) {
    convert_to_public_error = convert;
  }
  ...
  /// Convert to public error code.
  bool convert_to_public_error = false;
};
...
// Macros to 'finish async context' and also stop execution of the function with
// a 'return'. These macros are supposed to be used in a 'void' returning
// function such as a function executing in an asynchronous manner, on an async
// executor thread.
#define FINISH_CONTEXT_AND_RETURN(execution_result, context) \
  {                                                          \
    auto __execution_result = execution_result;              \
    FinishContext(__execution_result, context);              \
    return;                                                  \
  }

#define FINISH_CONTEXT_AND_RETURN_IF_FAILURE(execution_result, context) \
  {                                                                     \
    auto __execution_result = execution_result;                         \
    if (!__execution_result.Successful()) {                             \
      FinishContext(__execution_result, context);                       \
      return;                                                           \
    }                                                                   \
  }
