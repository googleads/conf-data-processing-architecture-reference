...
void AwsInstanceClientProvider::GetCurrentInstanceResourceName(
...
void AwsInstanceClientProvider::GetInstanceDetailsByResourceName(
...
void AwsInstanceClientProvider::OnDescribeInstancesAsyncCallback(
  ...
  for (const auto& tag : target_instance.GetTags()) {
    labels_proto->insert(MapPair<string, string>(tag.GetKey(), tag.GetValue()));
  }
...
void AwsInstanceClientProvider::GetTagsByResourceName(
...
void AwsInstanceClientProvider::OnDescribeTagsAsyncCallback(
  ...
  for (const auto& tag : outcome.GetResult().GetTags()) {
    tags->insert(MapPair<string, string>(tag.GetKey(), tag.GetValue()));
  }
