...
class GcpInstanceClientUtils {
 public:
  ...
  /**
   * @brief Convert the given instance_resource_name to InstanceName of format:
   * https://www.googleapis.com/compute/v1/projects/{PROJECT_ID}/zones/{ZONE_ID}/instances/{INSTANCE_ID}
   *
   * @param instance_resource_name the given instance_resource_name.
   * @return core::ExecutionResultOr<std::string> the convert result.
   */
  static core::ExecutionResultOr<std::string> ToInstanceName(
      const std::string& instance_resource_name) noexcept;

  static core::ExecutionResultOr<std::string> ExtractRegion(
      const std::string& zone) noexcept;
};
}  // namespace google::scp::cpio::client_providers
