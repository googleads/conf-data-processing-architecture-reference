...
void GcpInstanceClientProvider::GetCurrentInstanceResourceName(
  ...
  if (!execution_result.Successful()) {
    SCP_ERROR_CONTEXT(kGcpInstanceClientProvider, get_resource_name_context,
                      execution_result,
                      "Failed to perform http request to fetch project ID");
    return;
  }
  ...
  if (!execution_result.Successful()) {
    SCP_ERROR_CONTEXT(kGcpInstanceClientProvider, get_resource_name_context,
                      execution_result,
                      "Failed to perform http request to fetch zone ID");
    return;
  }
  ...
  if (!execution_result.Successful()) {
    SCP_ERROR_CONTEXT(kGcpInstanceClientProvider, get_resource_name_context,
                      execution_result,
                      "Failed to perform http request to fetch instance ID");
  }
}
...
void GcpInstanceClientProvider::GetTagsByResourceName(
...
void GcpInstanceClientProvider::OnGetTagsByResourceNameCallback(
  ...
  for (const auto& tag : json_response[kTagBindingsListKey]) {
    tags->insert(
        MapPair<string, string>(tag[kTagBindingNameKey].get<string>(),
...
void GcpInstanceClientProvider::GetInstanceDetailsByResourceName(
...
void GcpInstanceClientProvider::OnGetInstanceDetailsCallback(
  ...
  if (labels != json_response.end()) {
    ...
    for (json::iterator it = labels->begin(); it != labels->end(); ++it) {
      labels_proto->insert(
          MapPair<string, string>(it.key(), it.value().get<string>()));
    }
  }
