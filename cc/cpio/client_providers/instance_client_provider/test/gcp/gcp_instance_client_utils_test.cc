...
constexpr char kInstanceName[] =
    "https://www.googleapis.com/compute/v1/projects/123456789/zones/"
    "us-central1-c/instances/987654321";
...
TEST(GcpInstanceClientUtilsTest, ToInstanceNameFailedWithInvalidResourceName) {
  auto instance_id_or = GcpInstanceClientUtils::ToInstanceName("invalid");
  EXPECT_THAT(instance_id_or.result(),
              ResultIs(FailureExecutionResult(
                  SC_GCP_INSTANCE_CLIENT_INVALID_INSTANCE_RESOURCE_NAME)));
}

TEST(GcpInstanceClientUtilsTest, ToInstanceNameSucceeded) {
  auto instance_id_or =
      GcpInstanceClientUtils::ToInstanceName(kResourceNameMock);
  EXPECT_SUCCESS(instance_id_or.result());
  EXPECT_EQ(*instance_id_or, kInstanceName);
}

TEST(GcpInstanceClientUtilsTest, ExtractRegionFailedWithInvalidZone) {
  auto instance_id_or = GcpInstanceClientUtils::ExtractRegion("us-central");
  EXPECT_THAT(instance_id_or.result(),
              ResultIs(FailureExecutionResult(
                  SC_GCP_INSTANCE_CLIENT_ZONE_PARSING_FAILURE)));
}

TEST(GcpInstanceClientUtilsTest, ExtractRegionSucceeded) {
  auto region_or = GcpInstanceClientUtils::ExtractRegion(kZoneIdMock);
  EXPECT_SUCCESS(region_or.result());
  EXPECT_EQ(*region_or, "us-central1");
}
