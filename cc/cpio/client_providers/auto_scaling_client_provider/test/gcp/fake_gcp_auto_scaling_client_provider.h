...
class FakeInstanceGroupManagersClientFactory
    : public InstanceGroupManagersClientFactory {
 public:
  std::shared_ptr<cloud::compute_region_instance_group_managers_v1::
                      RegionInstanceGroupManagersClient>
  CreateClient(const std::shared_ptr<AutoScalingClientOptions>&
                   options) noexcept override;

  virtual ~FakeInstanceGroupManagersClientFactory() = default;
};

class FakeInstanceDatabaseClientProviderFactory
 ...
 public:
  ...
  virtual ~FakeInstanceDatabaseClientProviderFactory() = default;
};
}  // namespace google::scp::cpio::client_providers
