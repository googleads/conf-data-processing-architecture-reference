...
void AwsAutoScalingClientProvider::TryFinishInstanceTermination(
  ...
  if (!instance_id_or.result().Successful()) {
    try_termination_context.result = instance_id_or.result();
    SCP_ERROR_CONTEXT(
        kAwsAutoScalingClientProvider, try_termination_context,
        try_termination_context.result,
        "Failed to parse instance ID from the input instance resource name: "
        "%s.",
  ...
  auto instance_id = move(*instance_id_or);
  ...
  auto_scaling_client_->DescribeAutoScalingInstancesAsync(
      ...
      bind(
          ...
          this, try_termination_context, instance_id, _1, _2, _3, _4),
      nullptr);
}
...
void AwsAutoScalingClientProvider::OnDescribeAutoScalingInstancesCallback(
    ...
    const string& instance_id, const AutoScalingClient*,
  ...
  complete_lifecycle_action_request.SetInstanceId(instance_id);
...
shared_ptr<AutoScalingClient>
AwsAutoScalingClientFactory::CreateAutoScalingClient(
