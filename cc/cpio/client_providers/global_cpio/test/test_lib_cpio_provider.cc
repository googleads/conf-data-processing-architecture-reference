...
ExecutionResult TestLibCpioProvider::GetHttpClient(
    shared_ptr<HttpClientInterface>& http_client) noexcept {
  if (!test_options_->http_client) {
    return LibCpioProvider::GetHttpClient(http_client);
  }
  http_client = test_options_->http_client;
  return SuccessExecutionResult();
}

ExecutionResult TestLibCpioProvider::GetHttp1Client(
    shared_ptr<HttpClientInterface>& http1_client) noexcept {
  if (!test_options_->http_client) {
    return LibCpioProvider::GetHttpClient(http1_client);
  }
  http1_client = test_options_->http_client;
  return SuccessExecutionResult();
}

ExecutionResult TestLibCpioProvider::GetAuthTokenProvider(
    shared_ptr<AuthTokenProviderInterface>& auth_token_provider) noexcept {
  if (!test_options_->auth_token_provider) {
    return LibCpioProvider::GetAuthTokenProvider(auth_token_provider);
  }
  auth_token_provider = test_options_->auth_token_provider;
  return SuccessExecutionResult();
}
