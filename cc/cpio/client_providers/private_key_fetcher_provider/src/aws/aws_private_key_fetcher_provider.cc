...
void AwsPrivateKeyFetcherProvider::SignHttpRequest(
  ...
  request->account_identity = make_shared<string>(
      sign_request_context.request->key_endpoint->account_identity());
  ...
  role_credentials_provider_->GetRoleCredentials(
      get_session_credentials_context);
}
...
void AwsPrivateKeyFetcherProvider::
  ...
  execution_result = SignHttpRequestUsingV4Signer(
      ...
      *get_session_credentials_context.response->security_token,
      sign_request_context.request->key_endpoint->key_service_region());
