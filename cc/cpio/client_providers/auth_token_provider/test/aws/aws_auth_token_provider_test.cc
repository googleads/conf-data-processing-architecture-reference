...
class AwsAuthTokenProviderTest : public ScpTestBase,
...
TEST_F(AwsAuthTokenProviderTest,
  ...
  fetch_token_context_.callback = [&finished, &start_time](auto& context) {
    ...
    EXPECT_GE(context.response->expire_time.count(),
              start_time.seconds() + kTokenTtlInSecondHeaderValue);
    EXPECT_LE(context.response->expire_time.count(),
              end_time.seconds() + kTokenTtlInSecondHeaderValue);
    finished = true;
  };
