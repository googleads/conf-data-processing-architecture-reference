...
void AwsNoSQLDatabaseClientProvider::CreateTable(
    AsyncContext<CreateTableRequest, CreateTableResponse>&
        create_table_context) noexcept {
  // TODO implement
  create_table_context.result = FailureExecutionResult(SC_UNKNOWN);
  create_table_context.Finish();
}

void AwsNoSQLDatabaseClientProvider::DeleteTable(
    AsyncContext<DeleteTableRequest, DeleteTableResponse>&
        delete_table_context) noexcept {
  // TODO implement
  delete_table_context.result = FailureExecutionResult(SC_UNKNOWN);
  delete_table_context.Finish();
}

void AwsNoSQLDatabaseClientProvider::GetDatabaseItem(
...
void AwsNoSQLDatabaseClientProvider::CreateDatabaseItem(
  ...
  // Ttl is optional
  if (request.has_ttl()) {
    put_item_request.AddItem(
        move("Ttl"),
        move(Aws::DynamoDB::Model::AttributeValue().SetN(std::to_string(
            absl::ToUnixSeconds(absl::Now() +
                                absl::Seconds(request.ttl().seconds()) +
                                absl::Nanoseconds(request.ttl().nanos()))))));
  }

  if (!request.attributes().empty()) {
    for (auto attribute_index = 0; attribute_index < request.attributes_size();
         ++attribute_index) {
...
void AwsNoSQLDatabaseClientProvider::UpsertDatabaseItem(
