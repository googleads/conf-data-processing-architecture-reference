...
class GcpNoSQLDatabaseClientProviderTests : public testing::Test {
 protected:
  GcpNoSQLDatabaseClientProviderTests()
    ...
    ON_CALL(*gcp_database_factory_, CreateClients)
...
TEST_F(GcpNoSQLDatabaseClientProviderTests, InitWithNullCpuAsyncExecutor) {
  GcpNoSQLDatabaseClientProvider gcp_spanner(
      make_shared<NoSQLDatabaseClientOptions>("instance", "database",
                                              GetTableNameToKeysMap()),
      make_shared<NiceMock<MockInstanceClientProvider>>(), nullptr,
      make_shared<MockAsyncExecutor>(), gcp_database_factory_);

  EXPECT_THAT(gcp_spanner.Init(),
              ResultIs(FailureExecutionResult(
                  SC_NO_SQL_DATABASE_PROVIDER_UNRETRIABLE_ERROR)));
}

TEST_F(GcpNoSQLDatabaseClientProviderTests, InitWithNullIoAsyncExecutor) {
  GcpNoSQLDatabaseClientProvider gcp_spanner(
      make_shared<NoSQLDatabaseClientOptions>("instance", "database",
                                              GetTableNameToKeysMap()),
      make_shared<NiceMock<MockInstanceClientProvider>>(),
      make_shared<MockAsyncExecutor>(), nullptr, gcp_database_factory_);
...
TEST_F(GcpNoSQLDatabaseClientProviderTests,
       InitWithMissingGcpSpannerInstanceName) {
  GcpNoSQLDatabaseClientProvider gcp_spanner(
      make_shared<NoSQLDatabaseClientOptions>("", "database",
                                              GetTableNameToKeysMap()),
      make_shared<NiceMock<MockInstanceClientProvider>>(),
      make_shared<MockAsyncExecutor>(), make_shared<MockAsyncExecutor>(),
      gcp_database_factory_);
...
TEST_F(GcpNoSQLDatabaseClientProviderTests,
       InitWithMissingGcpSpannerDatabaseName) {
  GcpNoSQLDatabaseClientProvider gcp_spanner(
      make_shared<NoSQLDatabaseClientOptions>("instance", "",
                                              GetTableNameToKeysMap()),
      make_shared<NiceMock<MockInstanceClientProvider>>(),
      make_shared<MockAsyncExecutor>(), make_shared<MockAsyncExecutor>(),
      gcp_database_factory_);
...
TEST_F(GcpNoSQLDatabaseClientProviderTests, GetItemWithPartitionKeyOnly) {
  ...
  gcp_spanner_.GetDatabaseItem(get_database_item_context_);
...
TEST_F(GcpNoSQLDatabaseClientProviderTests, CreateItemWithPartitionKeyOnly) {
  ...
  gcp_spanner_.CreateDatabaseItem(create_database_item_context_);
...
TEST_F(GcpNoSQLDatabaseClientProviderTests,
  ...
  EXPECT_CALL(*returned_results, NextRow())
      .WillOnce(Return(MakeRow({{"Value", Value(Json(R"""(
                {
                "other_val": "10",
                "token_count": "999"
                }
  ...
  Mutation m = MakeInsertOrUpdateMutation(
      kPartitionLockTableName, {kPartitionLockPartitionKeyName, "Value"},
      Value("3"), Value(Json("{\"other_val\":\"10\",\"token_count\":\"1\"}")));
  EXPECT_CALL(*connection_, Commit(FieldsAre(_, UnorderedElementsAre(m), _)))
      .WillOnce(Return(CommitResult{}));
...
TEST_F(GcpNoSQLDatabaseClientProviderTests,
       UpsertItemNoAttributesWithExistingValueWithTtl) {
  upsert_database_item_context_.request->mutable_key()->set_table_name(
      kPartitionLockTableName);
  upsert_database_item_context_.request->mutable_key()
      ->mutable_partition_key()
...
TEST_F(GcpNoSQLDatabaseClientProviderTests, CreateItemWithTtl) {
  create_database_item_context_.request->mutable_key()
      ->mutable_partition_key()
      ->CopyFrom(MakeStringAttribute(kBudgetKeyPartitionKeyName, "3"));
  create_database_item_context_.request->mutable_key()
  ...
  create_database_item_context_.request->mutable_ttl()->set_seconds(1);

  EXPECT_CALL(*connection_, Commit(FieldsAre(_, _, _)))
      .WillOnce(Return(CommitResult{}));
