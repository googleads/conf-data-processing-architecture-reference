...
#include "cpio/client_providers/blob_storage_client_provider/src/aws/aws_blob_storage_client_utils.h"
...
ExecutionResult SetContentMd5(Context& context, Request& request,
  ...
  ASSIGN_OR_LOG_AND_RETURN_CONTEXT(string base64_md5_checksum,
                                   Base64Encode(md5_checksum), kAwsS3Provider,
                                   context, "Encoding MD5 to base64 failed");
...
ExecutionResult ValidateGetBlobRequest(Context& context) {
  ...
  if (request.blob_metadata().bucket_name().empty() ||
    ...
    SCP_ERROR_CONTEXT(kAwsS3Provider, context, failure,
  ...
  if (request.has_byte_range() && request.byte_range().begin_byte_index() >
    ...
    auto failure =
        FailureExecutionResult(SC_BLOB_STORAGE_PROVIDER_INVALID_ARGS);
    SCP_ERROR_CONTEXT(
        kAwsS3Provider, context, failure,
...
void AwsBlobStorageClientProvider::GetBlobStream(
    ConsumerStreamingContext<GetBlobStreamRequest, GetBlobStreamResponse>&
        get_blob_stream_context) noexcept {
  if (auto result = ValidateGetBlobRequest(get_blob_stream_context);
      !result.Successful()) {
    get_blob_stream_context.result = result;
...
void AwsBlobStorageClientProvider::ListBlobsMetadata(
...
void AwsBlobStorageClientProvider::PutBlob(
...
void AwsBlobStorageClientProvider::PutBlobStream(
...
void AwsBlobStorageClientProvider::DeleteBlob(
...
AwsBlobStorageClientProvider::GetBlobStreamSync(
    const BlobIdentity& blob_identity) noexcept {
  return FailureExecutionResult(SC_COMMON_ERRORS_UNIMPLEMENTED);
}

ExecutionResultOr<std::unique_ptr<std::ostream>>
AwsBlobStorageClientProvider::PutBlobStreamSync(
    const BlobIdentity& blob_identity) noexcept {
  return FailureExecutionResult(SC_COMMON_ERRORS_UNIMPLEMENTED);
}
