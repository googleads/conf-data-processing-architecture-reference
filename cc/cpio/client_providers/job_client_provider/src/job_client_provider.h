...
static constexpr int kGetJobRetryStrategyDelayInMs = 101;
static constexpr int kGetJobRetryStrategyMaxRetries = 8;
...
class JobClientProvider : public JobClientProviderInterface {
            ...
            core::common::RetryStrategy(core::common::RetryStrategyOptions{
                ...
                job_client_options
                    ? static_cast<core::TimeDuration>(
                          job_client_options->read_job_retry_interval.count())
                    : kGetJobRetryStrategyDelayInMs,
                job_client_options
                    ? job_client_options->read_job_max_retry_count
                    : kGetJobRetryStrategyMaxRetries})) {}
 ...
 protected:
  /**
   * @brief Validate JobClientOptions has all necessarily fields.
   * @param job_client_options the job client options.
   * @return core::ExecutionResult The validation result.
   */
  virtual core::ExecutionResult ValidateOptions(
      const std::shared_ptr<JobClientOptions>& job_client_options) noexcept;
 ...
 private:
   ...
   */
  void OnGetDatabaseItemForGetNextJobCallback(
      core::AsyncContext<cmrt::sdk::job_service::v1::GetNextJobRequest,
                         cmrt::sdk::job_service::v1::GetNextJobResponse>&
          get_next_job_context,
      std::shared_ptr<std::string> job_id,
