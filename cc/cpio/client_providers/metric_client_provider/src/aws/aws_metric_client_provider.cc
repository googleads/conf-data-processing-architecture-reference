...
ExecutionResult AwsMetricClientProvider::MetricsBatchPush(
  ...
  // Already confirmed if batch recording is enabled,
  // namespace_for_batch_recording is set. And if batch recording is not
  // enabled, metric_requests_vector should only have one entry.
  string name_space =
