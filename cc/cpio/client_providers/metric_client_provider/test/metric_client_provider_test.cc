...
class MetricClientProviderTest : public ScpTestBase {
 protected:
  ...
  shared_ptr<MetricClientOptions> CreateMetricClientOptions(
    ...
    options->namespace_for_batch_recording = metric_namespace;
...
TEST_F(MetricClientProviderTest, PutMetricSuccessWithMultipleThreads) {
  ...
  for (auto i = 0; i < 100; ++i) {
    threads.push_back(thread([&]() { client->PutMetrics(context); }));
  }
