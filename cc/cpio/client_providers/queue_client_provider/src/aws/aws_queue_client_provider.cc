...
using google::scp::core::errors::
    SC_AWS_QUEUE_CLIENT_PROVIDER_INVALID_MESSAGE_BODY;
using google::scp::core::errors::
...
using google::scp::core::errors::
    SC_AWS_QUEUE_CLIENT_PROVIDER_NO_MESSAGE_RETURNED;
using google::scp::core::errors::
...
void AwsQueueClientProvider::EnqueueMessage(
...
void AwsQueueClientProvider::GetTopMessage(
...
void AwsQueueClientProvider::OnReceiveMessageCallback(
  ...
  if (messages.size() == 0) {
    execution_result = FailureExecutionResult(
        SC_AWS_QUEUE_CLIENT_PROVIDER_NO_MESSAGE_RETURNED);
    SCP_ERROR_CONTEXT(kAwsQueueClientProvider, get_top_message_context,
                      execution_result,
                      "No messages are returned from the SQS. Queue Name: %d",
                      queue_client_options_->queue_name.c_str());
    FinishContext(execution_result, get_top_message_context,
                  cpu_async_executor_);
    return;
  }
  ...
  auto message_body = message.GetBody();
  if (message_body.empty()) {
    execution_result = FailureExecutionResult(
        SC_AWS_QUEUE_CLIENT_PROVIDER_INVALID_MESSAGE_BODY);
    SCP_ERROR_CONTEXT(kAwsQueueClientProvider, get_top_message_context,
                      execution_result,
                      "The message body in the message receiving from SQS is "
                      "empty. Queue Name: %d",
                      queue_client_options_->queue_name.c_str());
    FinishContext(execution_result, get_top_message_context,
                  cpu_async_executor_);
  ...
  response->set_message_body(message_body.c_str());
...
void AwsQueueClientProvider::UpdateMessageVisibilityTimeout(
...
void AwsQueueClientProvider::DeleteMessage(
