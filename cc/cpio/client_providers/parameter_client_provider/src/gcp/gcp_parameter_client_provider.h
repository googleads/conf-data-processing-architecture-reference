...
class GcpParameterClientProvider : public ParameterClientProviderInterface {
 public:
  ...
  GcpParameterClientProvider(
      ...
      std::shared_ptr<SecretManagerFactory> secret_manager_factory =
          std::make_shared<SecretManagerFactory>())
      : async_executor_(async_executor),
        io_async_executor_(io_async_executor),
        instance_client_provider_(instance_client_provider),
        options_(options),
        secret_manager_factory_(secret_manager_factory) {}
  ...
  void GetParameter(core::AsyncContext<
 ...
 private:
  ...
  std::shared_ptr<SecretManagerFactory> secret_manager_factory_;
};

class SecretManagerFactory {
 public:
  virtual std::shared_ptr<cloud::secretmanager::SecretManagerServiceClient>
  CreateClient(const std::shared_ptr<ParameterClientOptions>& options) noexcept;

  virtual ~SecretManagerFactory() = default;
};
}  // namespace google::scp::cpio::client_providers
