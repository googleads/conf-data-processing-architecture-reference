...
shared_ptr<InstanceServiceFactoryOptions> TestAwsBlobStorageServiceFactory::
  ...
  auto options = make_shared<TestAwsInstanceServiceFactoryOptions>(
      *AwsBlobStorageServiceFactory::CreateInstanceServiceFactoryOptions());
  options->region_config_label = TestClientConfigurationKeys_Name(
      TestClientConfigurationKeys::CMRT_TEST_BLOB_STORAGE_CLIENT_REGION);
  return options;
}
  ...
  auto execution_result = TryReadConfigString(
      ...
      TestClientConfigurationKeys_Name(
          TestClientConfigurationKeys::
              CMRT_TEST_BLOB_STORAGE_CLIENT_CLOUD_ENDPOINT_OVERRIDE),
