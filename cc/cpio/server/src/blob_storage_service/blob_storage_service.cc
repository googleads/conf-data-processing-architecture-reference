...
using google::cmrt::sdk::blob_storage_service::v1::ClientConfigurationKeys_Name;
...
class BlobStreamReadReactor
 ...
 private:
  ExecutionResult InitiateCall(
    ...
    blob_storage_client->PutBlobStream(context);
 ...
 private:
  ExecutionResult InitiateCall(
      ConsumerStreamingContext<GetBlobStreamRequest, GetBlobStreamResponse>
          context) noexcept override {
    blob_storage_client->GetBlobStream(context);
    return SuccessExecutionResult();
  }
};
...
class BlobStorageServiceImpl : public BlobStorageService::CallbackService {
 public:
  ...
  grpc::ServerUnaryReactor* GetBlob(grpc::CallbackServerContext* server_context,
    ...
    return ExecuteNetworkCall2<GetBlobRequest, GetBlobResponse>(
  ...
  grpc::ServerUnaryReactor* PutBlob(grpc::CallbackServerContext* server_context,
    ...
    return ExecuteNetworkCall2<PutBlobRequest, PutBlobResponse>(
        server_context, request, response,
  ...
  grpc::ServerUnaryReactor* ListBlobsMetadata(
    ...
    return ExecuteNetworkCall2<ListBlobsMetadataRequest,
  ...
  grpc::ServerUnaryReactor* DeleteBlob(
    ...
    return ExecuteNetworkCall2<DeleteBlobRequest, DeleteBlobResponse>(
        server_context, request, response,
  ...
  TryReadConfigInt(config_provider,
                   ClientConfigurationKeys_Name(
                       ClientConfigurationKeys::
                           CMRT_BLOB_STORAGE_CLIENT_COMPLETION_QUEUE_COUNT),
                   num_completion_queues);
  auto min_pollers = kDefaultMinPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_BLOB_STORAGE_CLIENT_MIN_POLLERS),
      min_pollers);
  auto max_pollers = kDefaultMaxPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_BLOB_STORAGE_CLIENT_MAX_POLLERS),
      max_pollers);
