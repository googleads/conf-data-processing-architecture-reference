...
#include "test_aws_job_service_factory.h"
...
TestAwsJobServiceFactory::CreateInstanceServiceFactory() noexcept {
...
TestAwsJobServiceFactory::CreateInstanceServiceFactoryOptions() noexcept {
  auto options = make_shared<TestAwsInstanceServiceFactoryOptions>(
      *AwsJobServiceFactory::CreateInstanceServiceFactoryOptions());
  options->region_config_label = TestClientConfigurationKeys_Name(
      TestClientConfigurationKeys::CMRT_TEST_JOB_CLIENT_REGION);
  return options;
}
}  // namespace google::scp::cpio
