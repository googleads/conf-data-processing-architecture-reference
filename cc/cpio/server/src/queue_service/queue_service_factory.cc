...
QueueServiceFactory::CreateQueueClientOptions() noexcept {
  ...
  client_options->queue_name = ReadConfigString(
      config_provider_,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_QUEUE_CLIENT_QUEUE_NAME));
...
QueueServiceFactory::CreateInstanceServiceFactoryOptions() noexcept {
  ...
  instance_service_factory_options
      ...
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_QUEUE_CLIENT_CPU_THREAD_COUNT);
  instance_service_factory_options->cpu_async_executor_queue_cap_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_QUEUE_CLIENT_CPU_THREAD_POOL_QUEUE_CAP);
  instance_service_factory_options
      ->io_async_executor_thread_count_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_QUEUE_CLIENT_IO_THREAD_COUNT);
  instance_service_factory_options->io_async_executor_queue_cap_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_QUEUE_CLIENT_IO_THREAD_POOL_QUEUE_CAP);
