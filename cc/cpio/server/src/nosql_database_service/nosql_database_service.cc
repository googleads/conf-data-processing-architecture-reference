...
class NoSqlDatabaseServiceImpl : public NoSqlDatabaseService::CallbackService {
 public:
  grpc::ServerUnaryReactor* GetDatabaseItem(
    ...
    return ExecuteNetworkCall2<GetDatabaseItemRequest, GetDatabaseItemResponse>(
  ...
  grpc::ServerUnaryReactor* CreateDatabaseItem(
    ...
    return ExecuteNetworkCall2<CreateDatabaseItemRequest,
  ...
  grpc::ServerUnaryReactor* UpsertDatabaseItem(
    ...
    return ExecuteNetworkCall2<UpsertDatabaseItemRequest,
  ...
  TryReadConfigInt(config_provider,
                   ClientConfigurationKeys_Name(
                       ClientConfigurationKeys::
                           CMRT_NOSQL_DATABASE_CLIENT_COMPLETION_QUEUE_COUNT),
                   num_completion_queues);
  auto min_pollers = kDefaultMinPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_NOSQL_DATABASE_CLIENT_MIN_POLLERS),
      min_pollers);
  auto max_pollers = kDefaultMaxPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_NOSQL_DATABASE_CLIENT_MAX_POLLERS),
      max_pollers);
