...
#include "public/cpio/proto/public_key_service/v1/configuration_keys.pb.h"
...
using google::cmrt::sdk::public_key_service::v1::ClientConfigurationKeys;
...
class PublicKeyServiceImpl : public PublicKeyService::CallbackService {
 public:
  grpc::ServerUnaryReactor* ListPublicKeys(
    ...
    return ExecuteNetworkCall2<ListPublicKeysRequest, ListPublicKeysResponse>(
  ...
  TryReadConfigInt(config_provider,
                   ClientConfigurationKeys_Name(
                       ClientConfigurationKeys::
                           CMRT_PUBLIC_KEY_CLIENT_COMPLETION_QUEUE_COUNT),
                   num_completion_queues);
  auto min_pollers = kDefaultMinPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_PUBLIC_KEY_CLIENT_MIN_POLLERS),
      min_pollers);
  auto max_pollers = kDefaultMaxPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_PUBLIC_KEY_CLIENT_MAX_POLLERS),
      max_pollers);
...
void RunClients() {
  ...
  if (!TryReadConfigInt(
           config_provider,
           ClientConfigurationKeys_Name(
               ClientConfigurationKeys::CMRT_PUBLIC_KEY_CLIENT_IO_THREAD_COUNT),
           io_thread_count)
           .Successful()) {
  ...
  if (!TryReadConfigInt(
           ...
           ClientConfigurationKeys_Name(
               ClientConfigurationKeys::
                   CMRT_PUBLIC_KEY_CLIENT_IO_THREAD_POOL_QUEUE_CAP),
  ...
  ReadConfigStringList(config_provider,
                       ClientConfigurationKeys_Name(
                           ClientConfigurationKeys::
                               CMRT_PUBLIC_KEY_KEY_VENDING_SERVICE_ENDPOINTS),
