...
namespace {
constexpr char kAutoScalingServiceFactory[] = "AutoScalingServiceFactory";
}  // namespace

namespace google::scp::cpio {
shared_ptr<InstanceServiceFactoryOptions>
AutoScalingServiceFactory::CreateInstanceServiceFactoryOptions() noexcept {
  ...
  instance_service_factory_options
      ...
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_AUTO_SCALING_CLIENT_CPU_THREAD_COUNT);
  instance_service_factory_options->cpu_async_executor_queue_cap_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::
              CMRT_AUTO_SCALING_CLIENT_CPU_THREAD_POOL_QUEUE_CAP);
  instance_service_factory_options
      ->io_async_executor_thread_count_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_AUTO_SCALING_CLIENT_IO_THREAD_COUNT);
  instance_service_factory_options->io_async_executor_queue_cap_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::
              CMRT_AUTO_SCALING_CLIENT_IO_THREAD_POOL_QUEUE_CAP);
  return instance_service_factory_options;
}

shared_ptr<AutoScalingClientOptions>
AutoScalingServiceFactory::CreateAutoScalingClientOptions() noexcept {
  return make_shared<AutoScalingClientOptions>();
}

ExecutionResult AutoScalingServiceFactory::Init() noexcept {
  ...
  if (!execution_result.Successful()) {
    SCP_ERROR(kAutoScalingServiceFactory, kZeroUuid, execution_result,
...
ExecutionResult AutoScalingServiceFactory::Run() noexcept {
...
ExecutionResult AutoScalingServiceFactory::Stop() noexcept {
