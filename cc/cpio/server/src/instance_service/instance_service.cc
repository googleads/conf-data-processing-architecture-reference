...
#include "public/cpio/proto/instance_service/v1/configuration_keys.pb.h"
...
using google::cmrt::sdk::instance_service::v1::ClientConfigurationKeys;
using google::cmrt::sdk::instance_service::v1::ClientConfigurationKeys_Name;
...
class InstanceServiceImpl : public InstanceService::CallbackService {
 public:
  grpc::ServerUnaryReactor* GetCurrentInstanceResourceName(
    ...
    return ExecuteNetworkCall2<GetCurrentInstanceResourceNameRequest,
                               GetCurrentInstanceResourceNameResponse>(
  ...
  grpc::ServerUnaryReactor* GetTagsByResourceName(
    ...
    return ExecuteNetworkCall2<GetTagsByResourceNameRequest,
  ...
  grpc::ServerUnaryReactor* GetInstanceDetailsByResourceName(
    ...
    return ExecuteNetworkCall2<GetInstanceDetailsByResourceNameRequest,
                               GetInstanceDetailsByResourceNameResponse>(
        server_context, request, response,
...
int main(int argc, char* argv[]) {
  ...
  TryReadConfigInt(
      ...
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_COMPLETION_QUEUE_COUNT),
      num_completion_queues);
  auto min_pollers = kDefaultMinPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_MIN_POLLERS),
      min_pollers);
  auto max_pollers = kDefaultMaxPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_MAX_POLLERS),
      max_pollers);
...
void RunClients() {
  ...
  instance_service_factory_options
      ...
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_CPU_THREAD_COUNT);
  instance_service_factory_options->cpu_async_executor_queue_cap_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::
              CMRT_INSTANCE_CLIENT_CPU_THREAD_POOL_QUEUE_CAP);
  instance_service_factory_options
      ->io_async_executor_thread_count_config_label =
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_IO_THREAD_COUNT);
  instance_service_factory_options
      ->io_async_executor_queue_cap_config_label = ClientConfigurationKeys_Name(
      ClientConfigurationKeys::CMRT_INSTANCE_CLIENT_IO_THREAD_POOL_QUEUE_CAP);
