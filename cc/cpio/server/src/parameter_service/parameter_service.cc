...
class ParameterServiceImpl : public ParameterService::CallbackService {
 public:
  grpc::ServerUnaryReactor* GetParameter(
    ...
    return ExecuteNetworkCall2<GetParameterRequest, GetParameterResponse>(
  ...
  TryReadConfigInt(config_provider,
                   ClientConfigurationKeys_Name(
                       ClientConfigurationKeys::
                           CMRT_PARAMETER_CLIENT_COMPLETION_QUEUE_COUNT),
                   num_completion_queues);
  auto min_pollers = kDefaultMinPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_PARAMETER_CLIENT_MIN_POLLERS),
      min_pollers);
  auto max_pollers = kDefaultMaxPollers;
  TryReadConfigInt(
      config_provider,
      ClientConfigurationKeys_Name(
          ClientConfigurationKeys::CMRT_PARAMETER_CLIENT_MAX_POLLERS),
      max_pollers);
