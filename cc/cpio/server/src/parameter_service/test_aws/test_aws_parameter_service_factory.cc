...
TestAwsParameterServiceFactory::CreateInstanceServiceFactoryOptions() noexcept {
  auto options = make_shared<TestAwsInstanceServiceFactoryOptions>(
      *AwsParameterServiceFactory::CreateInstanceServiceFactoryOptions());
  options->region_config_label = TestClientConfigurationKeys_Name(
      TestClientConfigurationKeys::CMRT_TEST_PARAMETER_CLIENT_REGION);
  return options;
}
  ...
  auto execution_result = TryReadConfigString(
      ...
      TestClientConfigurationKeys_Name(
          TestClientConfigurationKeys::
              CMRT_TEST_PARAMETER_CLIENT_CLOUD_ENDPOINT_OVERRIDE),
