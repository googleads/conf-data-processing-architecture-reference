...
public class HybridEncryptionKeyServiceImplTest {
  ...
  private HybridEncryptionKeyServiceImpl hybridEncryptionKeyServiceImpl;

  @Before
  public void setup() throws Exception {
    mockTinkUtils = new MockTinkUtils();
    hybridEncryptionKeyServiceImpl = new HybridEncryptionKeyServiceImpl(aead, keyFetchingService);
  }
  ...
  public void getDecrypter_getsDecrypter() throws Exception {
    ...
    HybridDecrypt actualHybridDecrypt =
        hybridEncryptionKeyServiceImpl.getDecrypter(UUID.randomUUID().toString());
  ...
  @Test
  public void getEncrypter_internalError_throwsMethodNotSupported() throws Exception {
    KeyFetchException exception =
        assertThrows(
            KeyFetchException.class,
            () -> hybridEncryptionKeyServiceImpl.getEncrypter(UUID.randomUUID().toString()));

    assertEquals(exception.getReason(), ErrorReason.INTERNAL);
    assertThat(exception.toString()).contains("Method not supported");
  }
