...
public final class JobClientImplTest {
  ...
  public void getJob_getsFakeJob() throws JobClientException {
    ...
    Optional<Job> actual = jobClient.getJob(GetJobRequest.builder().build());
  ...
  @Test
  public void getJob_marksJobWithExhaustedAttemptsAsFailed() throws Exception {
    ...
    GetJobRequest request =
        GetJobRequest.builder()
            ...
            .build();
    Optional<Job> actual = jobClient.getJob(request);
    ...
    assertThat(argument.getValue().messageBody()).contains(topicId);
  ...
  @Test
  public void getJob_marksJobWithExhaustedAttemptsAsFailed_sendNotificationToGlobalTopic()
      throws Exception {
    String internalTopicId = "internal_topic_id";
    when(parameterClient.getParameter(JOB_COMPLETION_NOTIFICATIONS_TOPIC_ID.name()))
        .thenReturn(Optional.of(internalTopicId));
  ...
  @Test
  public void markJobCompleted_marksJobCompletion_sendNotificationToCustomerTopic()
      throws Exception {
    String internalTopicId = "internal_topic_id";
    when(parameterClient.getParameter(JOB_COMPLETION_NOTIFICATIONS_TOPIC_ID.name()))
        .thenReturn(Optional.of(internalTopicId));
    ...
    JobResult result =
        JobResult.builder()
            ...
            .setTopicId(Optional.of(topicId))
            .build();
  ...
  @Test
  public void markJobCompleted_marksJobCompletion_sendNotificationToGlobalTopic() throws Exception {
    String internalTopicId = "internal_topic_id";
    when(parameterClient.getParameter(JOB_COMPLETION_NOTIFICATIONS_TOPIC_ID.name()))
        .thenReturn(Optional.of(internalTopicId));
    String globalTopicId = "global_topic_id";
  ...
  @Test
  public void
      markJobCompleted_marksJobCompletion_sendNotificationToCustomerTopicWithoutFlagEnabled()
          throws Exception {
    jobQueue.setJobQueueItemToBeReceived(Optional.of(baseJobQueueItem));
    jobMetadataDb.setJobMetadataToReturn(Optional.of(baseJobMetadata));
  ...
  @Test
  public void markJobCompleted_marksJobCompletion_sendNotificationToGlobalTopicWithoutFlagEnabled()
      throws Exception {
    String globalTopicId = "global_topic_id";
    when(parameterClient.getParameter(NOTIFICATIONS_TOPIC_ID.name()))
        .thenReturn(Optional.of(globalTopicId));
  ...
  @Test
  public void markJobCompleted_marksJobCompletion_noNotificationSent() throws Exception {
    jobQueue.setJobQueueItemToBeReceived(Optional.of(baseJobQueueItem));
    jobMetadataDb.setJobMetadataToReturn(Optional.of(baseJobMetadata));
    ...
    // make sure job is removed from the job queue
    assertThat(jobQueue.getLastJobQueueItemSent()).isEqualTo(baseJobQueueItem);
    verify(notificationClient, times(0)).publishMessage(any());
  }
  ...
  private static final class TestEnv extends AbstractModule {
    ...
    protected void configure() {
      ...
      parameterClient = mock(ParameterClient.class);
